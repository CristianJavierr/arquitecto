...
export default function App(){
 ...
 useEffect(()=>{let cancelled=false;const assets=Promise.all([document.fonts?.ready??Promise.resolve(),...['/images/leon-ferrer.jpg',...projects.map(item=>item.image)].map(waitForImage)]);const minimum=new Promise(resolve=>setTimeout(resolve,850));Promise.all([assets,minimum]).then(()=>{if(!cancelled)setAssetsReady(true)});return()=>{cancelled=true}},[])
  ...
  <main>
   ...
   <section className="architect-section" id="arquitecto" aria-labelledby="architect-title">
    <div className="architect-intro">
     <div className="architect-photo"><img src="/images/leon-ferrer.jpg" alt="León Ferrer en su estudio"/><span className="architect-photo-label">LEÓN FERRER / EL ESTUDIO</span></div>
     <div className="architect-copy"><span className="eyebrow">EL ARQUITECTO</span><MaskedLiftText as="h2" id="architect-title" trigger="scroll" delay={.08} duration={1.05} stagger={.1}>León<br/><em>Ferrer.</em></MaskedLiftText><MaskedLiftText as="p" trigger="scroll" delay={.2} duration={.95} stagger={.08}>León Ferrer trabaja entre la precisión del plano y la vida cotidiana. Su arquitectura empieza observando cómo entra la luz, cómo se recorre una habitación y qué lugar termina haciendo suyo cada persona.</MaskedLiftText><MaskedLiftText as="p" className="architect-note" trigger="scroll" delay={.32} duration={.85} stagger={.08}>Una práctica independiente de arquitectura, paisaje e interiores.</MaskedLiftText><div className="architect-facts"><span>FORMA / 2011</span><span>SANTO DOMINGO · MADRID</span></div></div>
    </div>
   ...
   <section className="contact-section" id="contacto" aria-labelledby="contact-title">
    ...
    <div className="contact-details"><MaskedLiftText as="p" trigger="scroll" delay={.2} duration={.95} stagger={.08}>Cuéntame qué estás imaginando, qué necesita tu espacio y en qué punto se encuentra la idea.</MaskedLiftText><a className="whatsapp-link" href="https://wa.me/18295550148?text=Hola%20Le%C3%B3n%2C%20quiero%20hablar%20sobre%20un%20proyecto" target="_blank" rel="noreferrer"><MaskedLiftText as="span" trigger="scroll" delay={.3} duration={.75} stagger={.06}>WhatsApp</MaskedLiftText><MaskedLiftText as="strong" trigger="scroll" delay={.38} duration={.9} stagger={.08}>+1 829 555 0148</MaskedLiftText><ArrowUpRight size={20}/></a><div className="contact-meta"><MaskedLiftText as="span" trigger="scroll" delay={.5} duration={.75} stagger={.06}>SANTO DOMINGO · MADRID</MaskedLiftText><MaskedLiftText as="span" trigger="scroll" delay={.56} duration={.75} stagger={.06}>RESPUESTA EN 24–48 H</MaskedLiftText></div></div>
   </section>
